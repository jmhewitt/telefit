...
public:
	...
	TSampler(Config &t_cfg) : Sampler(VECTOR, "T") {
		cfg = &t_cfg;
...
public:
	...
	BetaSampler(Config &t_cfg) : Sampler(VECTOR, "beta") { cfg = &t_cfg; }
...
public:
	...
	ThetaSampler(Config &t_cfg) : Sampler(VECTOR, "theta") {
		cfg = &t_cfg;
...
public:
	...
	SigmasqSampler(Config &t_cfg) : Sampler(REAL, "sigmasq") {
		cfg = &t_cfg;
...
public:
	...
	RWSampler(t_sd, t_cur, C, alpha, "rho", LOGIT, t_cfg.priors.L, t_cfg.priors.U) {
		cfg = &t_cfg;
		one = 1.0;
...
public:
	...
	SigmasqepsSampler(Config &t_cfg, double t_sd, double t_current, double C,
					  double alpha) :
	RWSampler(t_sd, t_current, C, alpha, "sigmasqeps", LOG) {
		cfg = &t_cfg;
